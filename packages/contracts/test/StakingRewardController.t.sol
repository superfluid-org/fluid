// SPDX-License-Identifier: MIT
pragma solidity ^0.8.23;

import { SFTest } from "./SFTest.t.sol";

import {
    ISuperToken,
    ISuperfluidPool
} from "@superfluid-finance/ethereum-contracts/contracts/interfaces/superfluid/ISuperfluid.sol";
import { SuperTokenV1Library } from "@superfluid-finance/ethereum-contracts/contracts/apps/SuperTokenV1Library.sol";

import { IStakingRewardController } from "../src/interfaces/IStakingRewardController.sol";

using SuperTokenV1Library for ISuperToken;

contract StakingRewardControllerTest is SFTest {
    // Units downscaler defined in StakingRewardController.sol
    uint128 private constant _UNIT_DOWNSCALER = 1e16;

    function setUp() public override {
        super.setUp();
    }

    function testUpdateStakerUnits(address caller, uint256 stakingAmount) external {
        vm.assume(caller != address(0));
        vm.assume(caller != address(_stakingRewardController.TAX_DISTRIBUTION_POOL()));
        stakingAmount = bound(stakingAmount, 1e16, 10_000_000e18);

        vm.prank(caller);
        vm.expectRevert(IStakingRewardController.NOT_APPROVED_LOCKER.selector);
        _stakingRewardController.updateStakerUnits(stakingAmount);

        vm.expectRevert(IStakingRewardController.NOT_LOCKER_FACTORY.selector);
        _stakingRewardController.approveLocker(caller);

        vm.prank(address(_fluidLockerFactory));
        _stakingRewardController.approveLocker(caller);

        vm.prank(caller);
        _stakingRewardController.updateStakerUnits(stakingAmount);

        assertEq(
            _stakingRewardController.TAX_DISTRIBUTION_POOL().getUnits(caller),
            stakingAmount / _UNIT_DOWNSCALER,
            "incorrect amount of units"
        );
    }

    function testRefreshSubsidyDistribution(address caller, uint256 stakingAmount, int96 flowRate) external {
        vm.assume(caller != address(0));
        vm.assume(caller != address(_stakingRewardController.TAX_DISTRIBUTION_POOL()));
        vm.assume(caller != address(_programManager));
        flowRate = int96(bound(flowRate, 1, 1e18));

        stakingAmount = bound(stakingAmount, 1e16, 10_000_000e18);

        // Fund the StakingRewardController
        vm.prank(FLUID_TREASURY);
        _fluidSuperToken.transfer(address(_stakingRewardController), 100_000e18);

        vm.prank(address(_fluidLockerFactory));
        _stakingRewardController.approveLocker(caller);

        vm.prank(caller);
        _stakingRewardController.updateStakerUnits(stakingAmount);

        assertEq(
            _stakingRewardController.TAX_DISTRIBUTION_POOL().getUnits(caller),
            stakingAmount / _UNIT_DOWNSCALER,
            "incorrect amount of units"
        );

        vm.prank(caller);
        vm.expectRevert(IStakingRewardController.NOT_PROGRAM_MANAGER.selector);
        _stakingRewardController.refreshSubsidyDistribution(flowRate);

        assertEq(
            _stakingRewardController.TAX_DISTRIBUTION_POOL().getMemberFlowRate(caller),
            0,
            "distribution flow rate should be 0"
        );

        vm.prank(address(_programManager));
        _stakingRewardController.refreshSubsidyDistribution(flowRate);

        (, int96 totalDistributionFlowRate) = _fluid.estimateFlowDistributionActualFlowRate(
            address(_stakingRewardController), _stakingRewardController.TAX_DISTRIBUTION_POOL(), flowRate
        );

        assertEq(
            _stakingRewardController.TAX_DISTRIBUTION_POOL().getMemberFlowRate(caller),
            totalDistributionFlowRate,
            "distribution flow rate should not be 0"
        );
    }
}
